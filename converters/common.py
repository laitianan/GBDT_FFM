...
def gen_feats(row):
    ...
    for j in range(1, 14):
        ...
        if value != '':
            value = int(value)
            ######当值大于2，则x=int(log(x^2)）否则拼接成 sp_x
            if value > 2:
...
####读取频率出现超过阈值的特征
def read__featsfreqent(threshold=10):
    frequent_feats = set()
...
def split(path, nr_thread, has_header):
    ...
    def open_with_header_witten(path, idx, header):
        ...
        f.write(header)
        return f
        ####使用Linux 命令 统计文件的行数，并计算好每个线程平均执行的文件大小
...
####把之前多个线程处理之后的离散文件合并同一个svm 格式文件之中
